...
class PerceptronTagger():
    ...
    def _get_features(self, i, word, context, prev, prev2):
        ...
        add('bias')
        add('i suffix', word[-3:])
        #add('i suffix-2', word[-2:]) #changes nothing
        add('i pref1', word[0])
        add('i pref2', word[:2]) #model 9 96.23
        add('i pref3', word[:3]) #model 10 96.29
        add('i-1 tag', prev)
        add('i-2 tag', prev2)
        ...
        # print(word, '|||', features)
        if prev == 'DET':
            features['i-1 det']+=1 # model1 metrics changed not, but at least "det" in line 18 was tagged right: DET instead of PRON, as before
        if prev2 == 'DET': #can be meaningful, since there can be an adjective between det and noun
            features['i-2 det'] += 1 #model2 96.13 instead of 96.10 , it works
        if prev == 'PRON':
            features['i-1 pron']+=1 #model6 96.16
        # if prev == 'AUX':
        #     features['i-1 aux']+=1 #model8 96.08
        # if word[:3] == 'her' or word[:3] == 'der': #usually ADV
        #     features['her or der']+=1 #model7 96.14 from 96.16
        # if prev == 'PART':
        #     features['i-1 part']+=1 #model3 96.08 from 96.13
        # if context[i - 1] == 'å': #before verbs only
        #     features['i-1 å']+=1 #model4 96.04 from 96.13
        # if prev == 'ADP':
        #     features['i-1 adp'] += 1 #model5 96.05
        return features
