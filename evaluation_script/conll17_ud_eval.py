    ...
    def alignment_score(alignment, key_fn, weight_fn=lambda w: 1):
        ...
        for i, words in enumerate(alignment.matched_words):
            if key_fn(words.gold_word, words.gold_parent) == key_fn(words.system_word, words.system_parent_gold_aligned):
                ...
                logging.info(i) #to see where mistakes are
...
def main():
    # Parse arguments
    logging.basicConfig(level=logging.INFO, filename='metrics.log',format='%(asctime)s %(message)s')
    ...
    if not args.verbose:
        ...
        logging.info("Metrics    | Precision |    Recall |  F1 Score | AligndAcc")
        logging.info("-----------+-----------+-----------+-----------+-----------")
        # for metric in metrics:
        #     logging.info("{:11}|{:10.2f} |{:10.2f} |{:10.2f} |{}".format(
        #         metric,
        ...
        logging.info("{:11}|{:10.2f} |{:10.2f} |{:10.2f} |{}".format(
            "UPOS",
            100 * evaluation["UPOS"].precision,
            100 * evaluation["UPOS"].recall,
            100 * evaluation["UPOS"].f1,
            "{:10.2f}".format(100 * evaluation["UPOS"].aligned_accuracy) if evaluation[
                                                                                "UPOS"].aligned_accuracy is not None else ""
        ))
